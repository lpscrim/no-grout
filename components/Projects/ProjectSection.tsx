...
export default function ProjectSection({
  ...
  return (
    ...
    >
      {/* Navigation buttons */}
      <button
        className="fixed xl:hidden top-[400] sm:top-[45%] right-4 z-50 text-background hover:text-accent transition-colors text-3xl sm:text-2xl font-semibold sm:font-normal  cursor-pointer"
        onClick={() => {
      ...
      <button
        className="fixed xl:hidden top-[500] sm:top-[55%] right-4 z-50 text-background hover:text-accent transition-colors text-3xl sm:text-2xl font-semibold sm:font-normal cursor-pointer"
        onClick={() => {
          if (fixedIdx === null) {
            scrollToProject(0);
      ...
      <div
        className={`absolute left-0 top-0 h-full z-20 transition-all duration-300 ${
          menuOpen ? "w-35 sm:w-40" : "w-3 sm:w-7 lg:w-10"
        }`}
      >
        <div
